...
- **Serving helpers** (`server/serving.ts`): `sanitizeLabel` strips internal codenames (Q.ai / UX Study) from all UI-bound strings; `siteKey` = slugified `city-state`; `toSessionItem` formats dates/times/pay labels. Prior project in `attached_assets/instawork-research-project/` is the reference (React frontend there not yet ported).
